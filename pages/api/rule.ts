...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    try {
      const response = await writeRuleReq(req.body)
  ...
  if (req.method === "PUT") {
    try {
      const response = await updateRuleReq(req.body)
      res.status(200).json({text: response})
    } catch (e) {
      console.log(e)
      res.status(500).json({text: "Something went wrong"})
  ...
  if (req.method === "DELETE") {
    try {
      const response = await deleteRuleReq(req.body)
      res.status(200).json({text: response})
    } catch (e) {
      console.log(e)
      res.status(500).json({text: "Something went wrong"})
...
async function writeRuleReq(data) {
  ...
  const action = {questionId: null, factId: null, factAction: null, theme: null};
