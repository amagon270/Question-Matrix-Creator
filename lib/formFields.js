...
export function FactCreateLayout(props) {
  ...
  return (
    <Form onSubmit={formSubmit} key="createFact">
      ...
      Fact Types
      {Dropdown("factType", makeDropdownable(factTypes, 'type', 'type'), ()=>{}, existingFact?.type)}
...
export function QuestionCreateLayout(props) {
  ...
  const firstRun = props.questionData.firstRun ?? true;
  props.questionData.firstRun = false;
  ...
  if (optionQuestionTypes.includes(questionData.questionType)) {
  ...
  if (sliderQuestionTypes.includes(questionData.questionType)) {
    var labelFields = [];
  ...
  return(
    <Form onSubmit={formSubmit} key="questionFields">
      ...
      Type
      {Dropdown("QuestionType", makeDropdownable(questionTypes, 'type', 'type'), handleQuestionTypeChange, existingQuestion?.type)}
      ...
      Fact: {facts.find(fact => fact.id == questionData.chosenFact)?.name} <br/>
      {SearchButtonMatrix(facts, "name", factField, clickFact, firstRun)}<br/>
...
export function RuleFields(props) {
  ...
  return (
    <Form onSubmit={formSubmit} key="RuleFields">
        ...
        Trigger Type
        {Dropdown("triggers", makeDropdownable(ruleTriggers, 'type', 'type'), ()=>{}, ruleData.rule?.triggerType)}
  ...
  function Test(order, data = null) {
    return (
      <Card key={"test"+order}>
        Fact
        {Dropdown(order+'-fact', makeDropdownable(facts, 'id', 'name'), handleTestChange, data?.factId)}
        Operator
        {Dropdown(order+'-operator', makeDropdownable(ruleOperations, 'type', 'type'), handleTestChange, data?.operation)}
