...
export function Dropdown(fieldName, labels, onChangeFunction, state, setState, name, defaultValue = null) {
  ...
  return (
    <div className="dropdown">
      ...
      {visibility ?
      <ul>
        {/* Allows for getting value on form submit.  Mostly there to handle old code */}
        <li><input id={fieldName} name={fieldName} onChange={onChangeFunction} contentEditable="false" value={selectedValue ?? ""} hidden></input></li>
        <li>
          <input 
            key="DropdownSearch" 
            id="dropdownSearch" 
  ...
  function QuestionOptionField(order, existingData = null) {
    return (
      <div key={"option"+order}>
        ...
        {Dropdown("Fact", makeDropdownable(facts, 'id', 'name'), ()=>{}, questionData, setQuestionData, "questionOptionFact", existingData?.fact)}
